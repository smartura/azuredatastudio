...
(function (globalThis, factory) {
	...
	async function load(modulePaths, resultCallback, options) {
		...
		loaderConfig.paths = {
			...
			'zone.js/dist/zone': `${baseNodeModulesPath}/zone.js/dist/zone`,
			'zone.js/dist/zone-error': `${baseNodeModulesPath}/zone.js/dist/zone-error`
		};
		...
		if (!safeProcess.sandboxed) {
			...
			loaderConfig.amdModulesPattern = /(vs|sql)\/|(^vscode-textmate$)|(^vscode-oniguruma$)|(^xterm$)|(^xterm-addon-search$)|(^xterm-addon-unicode11$)|(^xterm-addon-webgl$)|(^iconv-lite-umd$)|(^jschardet$)|(^@vscode\/vscode-languagedetection$)|(^tas-client-umd$)|(^ansi_up$)|(^zone.js\/dist\/zone$)|(^zone.js\/dist\/zone-error$)/;  // {{SQL CARBON EDIT}} include sql and ansi_up in regex
		}
