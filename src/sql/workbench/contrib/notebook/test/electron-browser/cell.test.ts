...
suite('Cell Model', function (): void {
	...
	test('cell should fire onCellPreviewChanged and onCellMarkdownModeChanged on edit', async function () {
		...
		let createCellPreviewPromise = () => {
			return new Promise((resolve, reject) => {
				setTimeout(() => reject(), 2000);
				cellModel.onCellPreviewModeChanged(showPreview => {
					resolve(showPreview);
				});
			});
		};

		let createMarkdownModeChangePromise = () => {
			return new Promise((resolve, reject) => {
				setTimeout(() => reject(), 2000);
				cellModel.onCellMarkdownModeChanged(showMarkdown => {
					resolve(showMarkdown);
				});
			});
		};
		...
		let cellPreviewPromise = createCellPreviewPromise();
		let cellMarkdownModeChangePromise = createMarkdownModeChangePromise();
		cellModel.isEditMode = true;

		let showPreview = await cellPreviewPromise;
		let showMarkdown = await cellMarkdownModeChangePromise;
		assert(showPreview);
		assert(showMarkdown);
